/* Mini-app v2 runtime: 4 tabs (Home / Connect / Router / Account), 8 themes, RU/EN.
...
(function () {
  ...
  // ---------- i18n ----------
  const RU = {
    tabHome: "Главная", tabConnect: "Подключение", tabRouter: "Роутер", tabAccount: "Кабинет",
    routerHint: "VPN для роутера — один тариф защищает всю домашнюю сеть, без установки приложений на каждое устройство.",
    routerEmpty: "Тарифы для роутера скоро появятся", routerEmptySub: "Мы уже их готовим — загляните позже.",
  ...
  const EN = {
    ...RU,
    tabHome: "Home", tabConnect: "Connect", tabRouter: "Router", tabAccount: "Account",
    routerHint: "VPN for your router — one plan protects the whole home network, no apps to install on every device.",
    routerEmpty: "Router plans coming soon", routerEmptySub: "We're setting them up — check back later.",
  ...
  function homeScreen() {
    ...
    const allPlans =
      salesMode === "constructor"
        ? []
        : ((state.plans && state.plans.items) || []).filter((p) => p.category !== "router");
  ...
  // Separate storefront for router-flashed plans (plans.category === "router") — same
  // purchase mechanics as Home, but its own tariff/duration selection so switching tabs
  // never resets what the user picked on the other one.
  function routerScreen() {
    const me = state.me;
    const sub = me && me.subscription;
    const usable = sub && ["active", "trial", "limited"].includes(sub.status);
    const routerPlans = ((state.plans && state.plans.items) || []).filter((p) => p.category === "router");
    const hint = [el("div", { class: "card fade" }, [el("div", { class: "sub", style: "font-size:13px", text: T.routerHint })])];
    const custom = customItems("router");
    if (routerPlans.length === 0) {
      return hint.concat(
        [
          el("div", { class: "card fade", style: "text-align:center" }, [
            el("div", { class: "h-cap", text: T.routerEmpty }),
            el("div", { class: "muted", style: "margin-top:6px", text: T.routerEmptySub }),
          ]),
        ],
        custom,
      );
    }
    const plan = routerPlans[state.routerTariffSel] || routerPlans[0];
    const durs = plan.durations;
    const selIdx = durs[state.routerPlanSel] ? state.routerPlanSel : 0;
    const sel = durs[selIdx];
    const base = durs[0] ? durs[0].price_minor / durs[0].days : 0;
    const card = el("div", { class: "card fade" }, [
      el("div", { class: "h-cap", text: T.choosePlan }),
      routerPlans.length > 1
        ? el(
            "div",
            { class: "chips", style: "margin-bottom:10px" },
            routerPlans.map((p, i) =>
              el("button", {
                class: `chip${(state.routerTariffSel || 0) === i ? " on" : ""}`,
                onclick: () => { state.routerTariffSel = i; state.routerPlanSel = 0; haptic(); render(); },
                text: p.name,
              }),
            ),
          )
        ...
        durs.map((d, i) => {
          ...
          return el(
            "div",
            {
              class: `plan-opt${i === selIdx ? " on" : ""}`,
              style: "position:relative",
              onclick: () => { state.routerPlanSel = i; haptic(); render(); },
            },
            [
              i === 1 ? el("span", { class: "badge", text: "★" }) : null,
              el("div", { class: "m", text: `${d.months} мес` }),
      ...
      el("button", {
        class: "btn primary",
        style: "margin-top:14px;" + btnStyle("renew"),
        onclick: () => purchase(plan, sel),
        text: `${btnText("renew", usable ? T.renew : T.buy)} · ${sel ? money(sel.price_minor) : ""}`,
      }),
    ]);
    return hint.concat([card], custom);
  }
  ...
  function render() {
    ...
    const frag =
      ...
      : state.tab === "connect" ? connectScreen()
      : state.tab === "router" ? routerScreen()
      : accountScreen();
  ...
  async function load() {
    try {
      ...
      if (tabParam && ["home", "connect", "router", "account"].includes(tabParam)) state.tab = tabParam;
