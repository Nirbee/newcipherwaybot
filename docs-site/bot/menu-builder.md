...
Хранится в настройке `CABINET_BUTTONS` (список через запятую — как `CONNECTION_APPS`), так что то же самое можно править и в [настройках](/cabinet/settings) текстом. Кнопки `balance` и `referral` дополнительно скрываются, если фича выключена (`BALANCE_ENABLED` / `REFERRAL_ENABLED`) — так отключённая функция не оставляет «мёртвую» кнопку. «Открыть приложение» и «‹ Меню» бот добавляет к экрану сам.
