...
from src.core.enums import Availability, Currency, PlanCategory, PlanType
...
class Plan(IntPk, TimestampMixin, Base):
    ...
    # Storefront bucket — lets the mini-app split plans across the Home tab (app) and the
    # separate "VPN for router" tab (router) without touching provisioning/pricing logic.
    category: Mapped[PlanCategory] = mapped_column(
        Enum(PlanCategory, native_enum=False, length=16), default=PlanCategory.APP
    )
